...
class Traceur : public EZWindow {
 Polynome  mon_polynome;  // remplace de fonction par mon POlynome 
  ...
  for(int i=0;i<getWidth();i+=pixel_step)
   {
    ...
    double       y = mon_polynome(x); // On declenche la fonction *f sur la coordonnee x et on stocke la valeur dans y.
...
public:
 Traceur(int w,int h,const char *name,const Polynome& _p,double _xmin,double _xmax,double _ymin,double _ymax,unsigned short _pixel_step)
  : EZWindow(w,h,name),
    mon_polynome(_p),xmin(_xmin),xmax(_xmax),ymin(_ymin),ymax(_ymax),pixel_step(_pixel_step),
    curi(-1),curj(-1)
...
class MyApplication : public EZDraw {
  double mes_coef[3] = {3, 4, 5}; 
  Polynome mon_polynome;
  Traceur wpoly;
 public:
  MyApplication()
   : mon_polynome(2 , mes_coef),
     wpoly   (640,480,"y=5x^2+4x+3", mon_polynome, -2. ,+2. , -5,5, 3)
  {} 
};
